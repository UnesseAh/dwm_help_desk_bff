...
export async function ticketListGet(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (order) {
            ...
            queryOptions.orderBy = { id: sortDirection };
        }
        ...
        if (user.role === 'ADMIN') {
            ...
            queryOptions.where = { serviceId: user.serviceId };
        }
        else if (user.role === 'USER') {
            queryOptions.where = { clientId: user.id };
        }
        else {
...
export async function statsTickets(req: Request, res: Response, next: NextFunction){
 try {
        const user = (req as any).user?.user;
        const {colStats} = req.query;
        const allowedStats = ['status' , 'priority'];

        if (!user) {
            return res.status(401).json({ error: "Unauthorized" });
        }

        if(!allowedStats?.includes(colStats as string)){
        ...
        const tickets = await prisma.ticket.groupBy(queryOptions); 
        const stats = Object.fromEntries(tickets.map(item => [item[colStats], item?._count?.id]));      
