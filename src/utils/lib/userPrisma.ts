...
export async function getListUsersPrisma(
  quearySearch: QuerySearch,
  paginate: boolean,
  limit: number = 20,
  page: number = 1,
) {
...
export async function changePasswordUserPrisma(
  id: number,
...
export async function statsUsersByRolePrisma() {
  ...
  const dataRolesStats = stats.reduce(
    (acc, item) => {
    ...
    {} as Record<
      string,
      { activated: number; notActivated: number; total: number }
    >,
  );
